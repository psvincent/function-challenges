// This pulls the file from the original doubleThePrice file.
const ordersMultiplier = require('./ordersMultiplier');

// Generally describes the test
describe('ordersMultiplier', () => {
    // "it" is what the test is going to be. it describes it more in detail.
    it ('for empty orders, return empty array', () => {
        const modifiedOrders = ordersMultiplier(2, []);
        expect (modifiedOrders).toEqual([]);
    })

    it ('for an order with one element, double that price when multiplier is two', () => {
        const modifiedOrders = ordersMultiplier(2, [{
            price: 10
        }]);
        expect (modifiedOrders).toEqual([{
            price: 10, 
            newPrice: 20
        }]);
    })

    it ('for an order with two elements, triple that price when the multiplier is three', () => {
        const modifiedOrders = ordersMultiplier(3, [{
            price: 10
        }, {
            price: 20
        }]);
        expect (modifiedOrders).toEqual([{
            price: 10, 
            newPrice: 30
        }, {
            price: 20, 
            newPrice: 60
        }]);
    })
})